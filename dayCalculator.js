function calculateDayInYear(date) {
  ...
  const validMonth = function(month) {
    return month && month >= 1 && month <= 12; // month = 12 must be possible
  }

  const validDay = function(month, day) {
    return day && day >= 1 && day <= DAYS_IN_MONTH[month - 1]; // day = daysinmonth must be possible
  }

  const calculateDayNumber = function(month, day) {
    let dayOfYear = day; // let dayofyear = day, not = 1
